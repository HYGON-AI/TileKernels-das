...
@T.macro
def divide_task_aligned(length: int, num_tasks: int, task_id: int, start: T.Ref, end: T.Ref, task_align: int):
    length_per_task = align(T.ceildiv(length, num_tasks), task_align)
    start = task_id * length_per_task
    end = T.min(start + length_per_task, length)
...
def get_get_fused_mapping_phase1_kernel(
    num_experts: int,
    num_topk: int,
    alignment: int,
    num_sms: int,
    ...
    @T.prim_func
    def get_fused_mapping_phase1_kernel(
        topk_idx: T.Tensor[(num_tokens, num_topk), T.int64],
        pos_to_expert: T.Tensor[(num_expanded_tokens,), T.int32],
        pos_to_token: T.Tensor[(num_expanded_tokens,), T.int32],
        pos_to_token_topk: T.Tensor[(num_expanded_tokens,), T.int32],
        ...
        with T.Kernel(num_sms, threads=num_threads) as (sm_idx,):
            ...
            divide_task_aligned(numel, num_global_warps, global_warp_idx, start, end, warp_size)
            for i in T.serial(start + lane_idx, end, warp_size):
                T.assume(0 <= i < numel)
                expert_idx = topk_idx_1d[i]
                if expert_idx != -1:
    ...
    return get_fused_mapping_phase1_kernel
...
def get_get_fused_mapping_phase2_kernel(
    num_experts: int,
    num_topk: int,
    alignment: int,
    num_sms: int,
    ...
    @T.prim_func
    def get_fused_mapping_phase2_kernel(
        topk_idx: T.Tensor[(num_tokens, num_topk), T.int64],
        pos_to_expert: T.Tensor[(num_expanded_tokens,), T.int32],
        pos_to_token: T.Tensor[(num_expanded_tokens,), T.int32],
        pos_to_token_topk: T.Tensor[(num_expanded_tokens,), T.int32],
        ...
        with T.Kernel(num_sms, threads=num_threads) as (sm_idx,):
            ...
            divide_task_aligned(numel, num_global_warps, global_warp_idx, start, end, warp_size)
            aligned_end = align(end, warp_size)
            lane_mask_bit = T.uint64(1) << T.uint64(lane_idx)
            lane_mask = lane_mask_bit + lane_mask_bit - T.uint64(1)
            lane_mask_rev = ~lane_mask
            for i in T.serial(start + lane_idx, aligned_end, warp_size):
                T.assume(0 <= i)
                expert_idx = T.Select(i < numel, T.int32(topk_idx_1d[i]), -1)
                mask = T.call_extern(T.uint64, 'tl::match_any_sync', T.uint64(0xFFFFFFFFFFFFFFFF), expert_idx)
                mask_prefix = mask & lane_mask
                mask_lo = T.uint32(mask_prefix & T.uint64(0xFFFFFFFF))
                mask_hi = T.uint32((mask_prefix >> T.uint64(32)) & T.uint64(0xFFFFFFFF))
                count = T.popcount(mask_lo) + T.popcount(mask_hi)
                if i < numel and expert_idx >= 0:
                    T.assume(expert_idx < num_experts)
                    prefix_count = experts_sum_per_warp_shared[warp_idx, expert_idx]
                    pos = prefix_count - count
    ...
    return get_fused_mapping_phase2_kernel
...
def get_fused_mapping(
    ...
    if torch.version.hip is not None:
        phase1_kernel = get_get_fused_mapping_phase1_kernel(num_experts, num_topk, alignment, num_sms)
        phase2_kernel = get_get_fused_mapping_phase2_kernel(num_experts, num_topk, alignment, num_sms)
        phase1_kernel(
            topk_idx,
            pos_to_expert,
            pos_to_token,
        ...
        if int(os.getenv('TK_PRINT_KERNEL_SOURCE', 0)):
            print(phase1_kernel.get_kernel_source())
            print(phase2_kernel.get_kernel_source())
    else:
